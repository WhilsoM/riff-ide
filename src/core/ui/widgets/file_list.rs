...
pub fn FileList(
    data: Rc<RefCell<Vec<EntryRc>>>,
    icons: Rc<IconsInteractionsStore>,
    on_item_click: Option<Rc<dyn Fn(&EntryRc) -> Option<UiAction>>>,
    pending_action: Rc<RefCell<Option<UiAction>>>,
) -> impl Component {
    ComponentWrapper::new(move |ui: &mut egui::Ui| {
        let entries = data.borrow().clone(); // clone Rc
...
fn render_entry(
    ...
    indent: usize,
    on_click: &Option<Rc<dyn Fn(&EntryRc) -> Option<UiAction>>>,
) -> Option<UiAction> {
    let mut action = None;
